...
from MOT_demo_v14_based_on_reid_v22_notstandard_yolov5_add_face_remove_skeleton_ref_v8 import *
...
def convert_track_to_stitch_format(split_each_track,mapping_node_id_to_bbox,mapping_node_id_to_features,split_each_track_valid_mask):
    ...
    return curr_predicted_tracks_bboxes_test,trajectory_node_dict,trajectory_idswitch_dict,trajectory_idswitch_reliability_dict

######################BO Optimization####################
def convert_track_to_bbox_list(split_each_track,mapping_node_id_to_bbox,trajectory_node_dict,trajectory_idswitch_dict,trajectory_idswitch_reliability_dict,mapping_node_id_to_features):
    ...
    uncertain_track_dict = {}
    uncertain_track_list = []
    node_id_to_list = {}
    for track_id in split_each_track:
        ...
        if trajectory_idswitch_dict[track_id] != []:
            if len(trajectory_idswitch_dict[track_id]) == 1: # 只有两段
                ...
                if heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[0] == heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[1]:
                    uncertain_track_list.append(track_id)
                    for idx,segment_reliability in enumerate(trajectory_idswitch_reliability_dict[track_id]):
                        if segment_reliability == heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[0]:
                            node_id_to_list[trajectory_node_dict[track_id][trajectory_segment[idx][0]]] = trajectory_node_dict[track_id][trajectory_segment[idx][0]:trajectory_segment[idx][-1]+1]

                    # uncertain_track_dict[track_id] = node_id_to_list
                    # uncertain_track_dict[track_id] =
        else:
        ...
        #mapping_frameid_to_bbox = {}
    #### 统计同一条轨迹10帧之内的相似度 ####
    # for track_id in predicted_tracks_bboxes_test:  # 同一条轨迹10帧之内最小iou相似度0.24
    #     if len(predicted_tracks_bboxes_test) <= 1:
    #         continue
    #     curr_trajectory = predicted_tracks_bboxes_test[track_id]
    #     nodes = list(curr_trajectory.keys())
    #     bbox1 = curr_trajectory[nodes[0]][1]
    #     bbox2 = curr_trajectory[nodes[-1]][1]
    #     iou_similarity = compute_iou_single_box([bbox1[0][1], bbox1[1][1], bbox1[0][0], bbox1[1][0]],
    #                            [bbox2[0][1], bbox2[1][1], bbox2[0][0], bbox2[1][0]])
    #
    #     print('track %s '%track_id,iou_similarity) # 不需要分割符号
    # 可能有新的轨迹被删掉了
    delete_track = set()
    uncertain_track_dict[0] = uncertain_track_list
    uncertain_track_dict[1] = node_id_to_list
    for track_id in range(len(predicted_tracks_centers)): #
        track_id += 1
        if len(predicted_tracks_centers[track_id]) <= 1:
            delete_track.add(track_id)
    return predicted_tracks_centers,predicted_tracks_bboxes,predicted_tracks_bboxes_test,delete_track,mapping_node_id_to_belonglings,trajectory_similarity_dict,uncertain_track_dict

def filter_out_invalid_mapping_node_id_to_belonglings(input_mapping_node_id_to_belongings,mapping_node_id_to_bbox,predicted_tracks_centers,predicted_tracks_bboxes,curr_batch_frame_node_list,delete_track,uncertain_track_dict):
    '''
    解决同一帧物体当中分配同一个id的问题
    '''
    global batch_stride
    unique_frame_list = sorted(np.unique([mapping_node_id_to_bbox[x][2] for x in mapping_node_id_to_bbox]))
    curr_last_frame_node_list = [x for x in mapping_node_id_to_bbox if mapping_node_id_to_bbox[x][2] == unique_frame_list[-1]]
    curr_first_frame_node_list = [x for x in mapping_node_id_to_bbox if mapping_node_id_to_bbox[x][2] == unique_frame_list[0]]
    stable_node_list = curr_first_frame_node_list + curr_last_frame_node_list
    ...
    for node_id in output_mapping_node_id_to_belongings:
        ...
        if len(output_mapping_node_id_to_belongings[node_id]) == 0:
            ...
            if node_id not in fix_node_list:
                fix_node_list.append(node_id)
            output_mapping_node_id_to_belongings[node_id] = track_list

    node_id_to_list = uncertain_track_dict[1]
    uncertain_track_node_list = list(node_id_to_list.keys())
    # fix_node_list += uncertain_track_node_list
    ...
    fix_node_information = {}
    for x in fix_node_list:
        fix_node_information[x] = mapping_node_id_to_bbox[x]
    ...
    ## 所有多于两个点的轨迹并且不在不确定的轨迹当中
    common_tracks = [track_id for track_id in predicted_tracks_centers if len(predicted_tracks_centers[track_id])>1 and track_id not in uncertain_track_dict[0]]
    ## 去掉点为1的轨迹(前面轨迹当中)
    ## 还会有轨迹当中一个绝对正确的点都没有
    ...
    # 打印出 node_track_mapping_matrix每行最小值

    print(len(np.min(node_track_mapping_matrix,axis=1)))
    print(np.min(node_track_mapping_matrix,axis=1))
    unmatched_node_list = []
    for node_id in fix_node_list:  # 也有可能common tracks当中本身没有问题
        ...
        #     output_mapping_node_id_to_belongings[node_id] = [0]
        ####  考虑需要修正的轨迹段 ####
        # if min(node_track_mapping_matrix[fix_node_list.index(node_id),:]) > 1000 and node_id not in uncertain_track_node_list:
        #     output_mapping_node_id_to_belongings[node_id] = [0]
        # elif min(node_track_mapping_matrix[fix_node_list.index(node_id),:]) > 1000 and node_id in uncertain_track_node_list:
        #     invalid_index = np.argwhere(np.array(node_track_mapping_matrix[fix_node_list.index(node_id),:]) >= 25).flatten() #需要去掉的索引
        #     start_node_list = list(set(output_mapping_node_id_to_belongings[node_id])-set([common_tracks[x] for x in invalid_index if common_tracks[x] in output_mapping_node_id_to_belongings[node_id]]))
        #     # start_node_list为空怎么办
        #     for node in node_id_to_list[node_id]: # 以该节点开始的节点
        #         output_mapping_node_id_to_belongings[node] = start_node_list
        # else:
        ...
        invalid_index = np.argwhere(np.array(node_track_mapping_matrix[fix_node_list.index(node_id),:]) >= 50).flatten() #需要去掉的索引
        ...
        # if len(output_mapping_node_id_to_belongings[node_id]) < 1 and node_id not in stable_node_list:    ### 如果与其他轨迹相似度都很低可能是新出现的轨迹 ##
        #     output_mapping_node_id_to_belongings[node_id] = [0]
        #     unmatched_node_list.append(mapping_node_id_to_bbox[node_id])
        # elif len(output_mapping_node_id_to_belongings[node_id]) < 1 and node_id in stable_node_list:
        #     # 把该点之后的点都串起来
        #     output_mapping_node_id_to_belongings[node_id] = list([common_tracks[np.argmin(node_track_mapping_matrix[fix_node_list.index(node_id),:])]])
        #
    ...
    fixed_node_belongings = [output_mapping_node_id_to_belongings[x] for x in fix_node_list]
    ...
    for frame_name in curr_batch_frame_node_list:
        ...
        #             output_mapping_node_id_to_belongings[node_id] = possible_choice
        ...
        for conflict_pair in conflict_node:
            # fix_node_list.append([conflict_pair[0],conflict_pair[1]])
            ...
            if conflict_pair[0] in fix_node_list and conflict_pair[1] in fix_node_list: # 找出当中node_mapping_更小的值
                if output_mapping_node_id_to_belongings[conflict_pair[0]][0] in common_tracks and output_mapping_node_id_to_belongings[conflict_pair[1]][0] in common_tracks: # 同时都在common_tracks才能比较
    ...
    # fix_node_list = np.unique(fix_node_list).tolist()
    fixed_node_belongings = [output_mapping_node_id_to_belongings[x] for x in fix_node_list] # 需要修正的点修正之后的可能属于集合

    return output_mapping_node_id_to_belongings,fix_node_list
...
def cluster_fix(result, mapping_edge_id_to_cost, mapping_node_id_to_bbox, mapping_node_id_to_features, device, source,tracklet_len):
    ...
    current_video_segment_predicted_tracks_bboxes_test_SSP, trajectory_node_dict, trajectory_idswitch_dict, trajectory_idswitch_reliability_dict = convert_track_to_stitch_format(
        split_each_track, mapping_node_id_to_bbox, mapping_node_id_to_features, split_each_track_valid_mask)
    ...
    for track in trajectory_idswitch_reliability_dict:
        if trajectory_idswitch_reliability_dict[track][0] >= tracklet_len-1 and len(trajectory_idswitch_reliability_dict[track])==1:  # if the tracklet is complete and has no idswitch
    ...
    for node in indefinite_node:
        width = mapping_node_id_to_bbox[node][0][1][0] - mapping_node_id_to_bbox[node][0][0][0]
        height = mapping_node_id_to_bbox[node][0][1][1] - mapping_node_id_to_bbox[node][0][0][1]
        aspect_ratio = height / width
        if aspect_ratio > statistic_information_dict['aspect ratio'][1] or aspect_ratio < statistic_information_dict['aspect ratio'][0]:
            remove_node_list.append(node) # 使用remove之后改变了indefinite_node 从而导致索引改变
            continue
        elif width > statistic_information_dict['width'][1] or width < statistic_information_dict['width'][0]:
            remove_node_list.append(node)
            continue
        elif height > statistic_information_dict['height'][1] or height < statistic_information_dict['height'][0]:
            remove_node_list.append(node)
            continue
    indefinite_node = list(set(indefinite_node)-set(remove_node_list))
    indefinite_node.sort()  # sort in order
    n_clusters = len(remained_tracks)  # cluster number
    ...
    initial_medoids = kmeans_plusplus_initializer(sample, n_clusters,random_state = 1).initialize(return_index=True)  # the initialization is different with k-means
    initial_medoids1 = kmeans_plusplus_initializer(sample, n_clusters,random_state = 2).initialize(return_index=True)  # the initialization is different with k-means
    ...
    kmedoids_instance = kmedoids(sample, initial_medoids, metric=my_metric)
    # run cluster analysis and obtain results
    kmedoids_instance.process()
    clusters_raw = kmedoids_instance.get_clusters()
    clusters = []
    clusters = [i for i in clusters_raw if len(i) > 1]
    ...
    # 调整indefinite_node_list ###
    indefinite_node = list(set(indefinite_node)-set([indefinite_node[i[0]] for i in clusters_raw if len(i) == 1]))
    ...
    for track_id in list(split_each_track.keys()):
        ...
        elif track_id not in cluster_tracks:
            continue
...
def BO_fix_Thompson_sampling(result, mapping_edge_id_to_cost, mapping_node_id_to_bbox, mapping_node_id_to_features, device, source,tracklet_len):
    ...
    # 使用SSP的轨迹结果
    # trajectory_idswitch_dict 对应于在node_list当中的索引
    current_video_segment_predicted_tracks_bboxes_test_SSP,trajectory_node_dict,trajectory_idswitch_dict,trajectory_idswitch_reliability_dict = convert_track_to_stitch_format(split_each_track, mapping_node_id_to_bbox, mapping_node_id_to_features,split_each_track_valid_mask)
    # collect invalid nodes and edges, then determine possible traj belongings of each node in mapping_node_id_to_bbox
    # 对mapping_node_id_to_belongings进行简化
    curr_batch_frame_node_list = {}  # dict,包含每帧包含节点id
    ##### start clustering for the remained indefinite nodes ###### 
    ...
    indefinite_node.sort() # sort in order
    n_clusters = len(remained_tracks) # cluster number
    reid_matrix = np.array([np.array(mapping_node_id_to_features[x]) for x in indefinite_node]) # each row is a person reid feature eg:(49,512)
    scaler = StandardScaler()
    normed_reid_data = scaler.fit_transform(reid_matrix)
    bbox_matrix = np.array([np.array(mapping_node_id_to_bbox[x][0]).flatten() for x in indefinite_node]) # each row is a person bbox information eg:(49,4)
    scaler_bbox = StandardScaler()
    normed_bbox_data = scaler_bbox.fit_transform(bbox_matrix)
    pca = PCA(n_components=0.9) 
    reid_pca = pca.fit_transform(normed_reid_data)
    ...
    sample = data
    # 2. Prepare initial centers using K-Means++ method.
    initial_medoids = kmeans_plusplus_initializer(sample, n_clusters).initialize(return_index=True) # the initialization is different with k-means
    # 3. create metric that will be used for clustering
    def my_distance(point1,point2):
        dimension = len(point1)
        result = 0.0
        ...
        for frame_name in cluster_frame_list:
            ...
            for bboxid in indefinite_node:
                if mapping_node_id_to_bbox[bboxid][2] == frame_name:
                    ...
                    # cv2.putText(curr_img, str(getDictKey_1(cluster_tracks,bboxid)), (int((left+right)/2), int((top+bottom)/2)), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
                    cv2.putText(curr_img, str(getDictKey_1(cluster_tracks,bboxid)), (left, top), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
                    cv2.rectangle(curr_img,(left,top),(right,bottom),(0,255,0),3)
            if not os.path.exists(os.path.join(source.split(source.split('/')[-1])[0], 'results_all',source.split('/')[-1] + '_cluster_results/')):
                os.makedirs(os.path.join(source.split(source.split('/')[-1])[0], 'results_all',source.split('/')[-1] + '_cluster_results/'))
            cv2.imwrite(os.path.join(source.split(source.split('/')[-1])[0], 'results_all',source.split('/')[-1] + '_cluster_results/') + frame_name, curr_img)
    show_clusters(cluster_tracks)
    # kmeans_visualizer.show_clusters(sample, clusters, final_centers)
    ##### 整理结果并且返回 #####
    split_each_track_refined = {}
...
#         if remained_tracks[(np.unique(y_pred).tolist()).index(label)] not in cluster_tracks:
#             cluster_tracks[remained_tracks[(np.unique(y_pred).tolist()).index(label)]] = []
#
#         cluster_tracks[remained_tracks[(np.unique(y_pred).tolist()).index(label)]].append([indefinite_node[idx],mapping_node_id_to_bbox[indefinite_node[idx]]])
    # define functions
    ...
    for frame_idx in range(int(data_dimension)):
        ...
        for unique_id_order_list_item in unique_id_order_list:
            ...
            node_to_add = [x for x in mapping_node_id_to_bbox if mapping_node_id_to_bbox[x][2] == frame_list[frame_idx]][unique_id_order_list.index(unique_id_order_list_item)] # 不止1个0,索引只能索引到第1个
