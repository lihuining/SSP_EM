                ...
                if heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[0] == heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[1]:
                    ...
                    for idx,segment_reliability in enumerate(trajectory_idswitch_reliability_dict[track_id]):
                        if segment_reliability == heapq.nlargest(2,trajectory_idswitch_reliability_dict[track_id])[0]:
                            ...
                            mapping_node_id_to_belonglings[trajectory_node_dict[track_id][trajectory_segment[idx][0]]] += possible_tracks
...
def cluster_fix(result, mapping_edge_id_to_cost, mapping_node_id_to_bbox, mapping_node_id_to_features, device, source,tracklet_len):
    ...
    def show_single_tracks(tracks_SSP):
        ...
        #cluster_frame_list = sorted(np.unique([tracks[x].values()[2] for x in tracks]))
        cluster_frame_list = sorted(np.unique([list(tracks[x].values())[0][0] for x in tracks]))

        indefinite_node = sorted(np.unique([list(tracks[x].keys())[0] for x in tracks]))
        for frame_name in cluster_frame_list:
            ...
            for bboxid in indefinite_node:
                if mapping_node_id_to_bbox[bboxid][2] == frame_name:
                    ...
                    cv2.putText(curr_img, str(getDictKey_1(tracks, bboxid)), (left, top),
    ...
    for node in indefinite_node:
        ...
        if aspect_ratio > statistic_information_dict['aspect ratio'][1] or aspect_ratio < \
                statistic_information_dict['aspect ratio'][0]:
    ...
    print('the number of indefinite node:',len(indefinite_node))
    n_clusters = len(remained_tracks) + cnt0 # cluster number
    ...
    initial_medoids = kmeans_plusplus_initializer(sample, n_clusters, random_state=1).initialize(
        return_index=True)  # the initialization is different with k-means
    ...
    def my_distance(point1, point2):
        ...
        #     result += abs(point1[i] - point2[i]) ** 2
        #
        # for i in range(dimension -5,dimension-1):
        #     result += 2*abs(point1[i] - point2[i]) ** 2
