...
class GMC:
    def __init__(self, method='sparseOptFlow', downscale=2, verbose=None):
        ...
        self.method = method # file
        self.downscale = max(1, int(downscale)) # 2
        ...
        if self.method == 'orb':
            ...
            #seqname = verbose[0]
            #self.gmc_file = open(r'tracker/GMC_files/MOT16/'+'GMC-'+str(seqname)+'.txt', 'w')
            ...
            self.gmcFile = open(filePath + "/GMC-" + seqName + ".txt", 'r') # '../tracker/GMC_files/MOTChallenge/GMC-MOT20-01.txt'
            self.gmcData = self.gmcFile.readlines()
    ...
    def applyFile(self, frame_cnt, detections=None):
        line = self.gmcData[frame_cnt]
        tokens = line.split("\t")
        H = np.eye(2, 3, dtype=np.float_)
        H[0, 0] = float(tokens[1])
        H[0, 1] = float(tokens[2])
