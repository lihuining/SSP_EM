...
def dti(txt_path, save_path, n_min=25, n_dti=20):
    ...
    for seq_txt in seq_txts:
        seq_name = seq_txt.split('/')[-1] # MOT20-01.txt
        seq_data = np.loadtxt(seq_txt, dtype=np.float64, delimiter=',')
        min_id = int(np.min(seq_data[:, 1])) # 最小id
        max_id = int(np.max(seq_data[:, 1])) # 最大id
        ...
        for track_id in range(min_id, max_id + 1):
            ...
            if tracklet.shape[0] == 0:
                continue
            n_frame = tracklet.shape[0] # tracklet跨越的帧数
        ...
        seq_results = seq_results[seq_results[:, 0].argsort()] # 对第1列元素按照从小到大排序，argsort()取出的是索引
...
if __name__ == '__main__':
    ...
    data_root = '/home/allenyljiang/Documents/Dataset/MOT20/train'
    txt_path = '/home/allenyljiang/Documents/SSP_EM/YOLOX_outputs/MOT20'
    save_path = '/home/allenyljiang/Documents/SSP_EM/YOLOX_outputs/MOT20/interpolate'
    
    mkdir_if_missing(save_path)
    #dti(txt_path, save_path, n_min=10, n_dti=20)
